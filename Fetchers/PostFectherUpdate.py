...
if unit == "minutes":
    loop_limit = valueTime
else:
    loop_limit = valueTime*60
...
def path_to_save(init_path):
    ...
    if os.path.exists(init_path):
        ...
        new_dir_name = f"data_{datetime.now().strftime('%H-%M')}"
...
while loop_limit > 0:
    print(f"Il reste {heure} minutes")
    heure -= 5
    for section_name in config.sections():
        directory = ""
        for key, value in config.items(section_name):
            directory = path_to_save(dir_name) + "/" + key
            print("entrer avec "+key)
            print(directory)
