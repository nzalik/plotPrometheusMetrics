...
for json_data in data_list:
    ...
    timestamps = np.array([int(ts) for ts, _ in result["values"]])
    values = [float(value) for _, value in result["values"]]

    #values = [float(x[1]) for x in result["values"]]

    given_value = 1718616554.704

    greater_than_value = timestamps[timestamps > given_value]

    last_ten_values = values[-(len(greater_than_value)):]

    timestamps = greater_than_value

    values = last_ten_values
...
plt.title('CPU Consumption per Service')
plt.xticks(rotation=45)
plt.legend()

lastEl = ticks_seconds4[-1]
